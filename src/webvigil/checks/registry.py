...
Built-in checks register with ``@register`` at import time. Third-party checks are found
through the ``webvigil.checks`` entry-point group.
"""
...
def unknown_check_ids(disabled: Sequence[str]) -> list[str]:
    """The subset of ``disabled`` that does not match any registered check id."""
    return sorted(set(disabled) - set(_REGISTRY))


def iter_checks(*, mode: ScanMode, disabled: Sequence[str] = ()) -> list[type[Check]]:
    """Registered checks eligible for a run in ``mode``, minus the ``disabled`` ids."""
