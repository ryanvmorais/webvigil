...
``ScanContext.forms`` and the crawled responses' ``Set-Cookie`` headers — no
extra request — and flags every state-changing (``POST``) form that carries no
anti-CSRF token, weighting the finding by the session cookie's ``SameSite``.
