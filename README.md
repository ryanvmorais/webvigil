...
> `v0.11`; request-envelope injection in `v0.12` (XXE opt-in `--xxe`). **Blind SSRF, truly
> blind command injection, blind XXE, and HTTP request smuggling are not on the roadmap** —
> each needs an out-of-band collaborator (a server the scanner hosts and the target calls
> back to) or raw-socket control, which the "engine talks only to the target" rule and the
> repository-only distribution rule out. Pair WebVigil with your own collaborator (Burp
> Collaborator, interactsh) for the blind cases. Automated login-form flows and
